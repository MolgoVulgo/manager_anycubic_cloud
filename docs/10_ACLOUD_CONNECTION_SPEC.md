...
### Headers requis (OBSERVED(CODE-LEGACY))
Les appels sous `/p/p/workbench/api/...` nécessitent les headers `XX-*` (mêmes conventions que le web UI).

Headers minimaux :
- `XX-Device-Type: web`
- `XX-IS-CN: 2`
- `XX-Version: <client_version>`
- `XX-Nonce: <uuid>`
- `XX-Timestamp: <epoch_ms>`
- `XX-Signature: <md5_signature>`
- `XX-Token: <token>` (si disponible)

#### Signature `XX-Signature` (OBSERVED(CODE-LEGACY))
Formule observée (équivalente JS) :
`md5(app_id + timestamp_ms + version + app_secret + nonce + app_id)`

Notes :
- `app_id`, `app_secret`, `version` sont dérivés du bundle web (donc **configurables**, pas “magiques” en dur).
- Si `XX-Token` manque, certains endpoints peuvent fonctionner via cookies, mais le mode robuste est avec `XX-Token`.
...
- Pas de retry aveugle sur `401/403`.
- Comportement actuel: une tentative contrôlée de récupération auth peut être lancée sur `401/403`
  (re-login via token disponible), puis la requête initiale est rejouée une seule fois.
