...
### Périmètre
- Onglet `Files` construit dans `gui/tabs/files_tab.py`.
- Boutons toolbar + boutons des cartes fichier.

### Entrées
- Session cloud active.
- Callback de refresh (`on_refresh`) injecté par `gui/app.py`.
...
3. Implémenter ensuite les actions réelles dans cet ordre: `Download` -> `Delete` -> `Details` -> `Print` -> `Upload`.
4. Réutiliser les endpoints déjà documentés dans `26_ANYCUBIC_CLOUD_ENDPOINTS_CATALOG.md` et les règles de lecture dans `27_ENDPOINT_RESPONSES_INTERPRETATION_SPEC.md`.
