...
def decode_pw0_nonzero_mask(
    ...
    *,
    ...
    selected = normalize_pw0_variant(variant)
    if selected == PW0_VARIANT_WORD16:
        out = _decode_word16_nonzero_mask(blob=blob, pixel_count=pixel_count, strict=strict)
    else:
        out = _decode_byte_token_nonzero_mask(blob=blob, pixel_count=pixel_count, strict=strict)
...
def _decode_word16_nonzero_mask(
    *,
    blob: bytes,
    pixel_count: int,
    strict: bool,
    ...
    for word_index in range(word_count):
        ...
        out[pixel_pos : pixel_pos + applied] = 255 if color_index != 0 else 0
        pixel_pos += applied
...
def _decode_byte_token_nonzero_mask(
    *,
    blob: bytes,
    pixel_count: int,
    strict: bool,
    ...
    while idx < size:
        ...
        if run_end <= pixel_pos:
            continue
        out[pixel_pos:run_end] = material
        pixel_pos = run_end
